...
export default function PreOrderInventoryPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (user?.permissions?.preOrders) {
                const productsData = await getPreOrderItems();
    ...
    if (!isAuthorized) {
        return (
            <div className="flex h-[50vh] flex-col items-center justify-center gap-4 text-center">
                ...
                <p className="text-muted-foreground">You do not have permission to view pre-order inventory.</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-8 p-2">
            ...
            <PreOrderInventoryGrid products={products} onRefresh={fetchData} />
        </div>
    );
}
